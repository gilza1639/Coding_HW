...
Если я правильно понял, то сумму мы можем сделать либо с помощью int / float, list, tuple, set, froznset и во внутренних вложениях
...
def list_counter(my_list):
    countered_list = []
    for el in my_list:
        countered_list.append(unpacker_and_counter(el))
    print('Start\n')
    for id, el in enumerate(my_list):
        print(f'{el} \t = \t {countered_list[id]}')
    countered_list.sort()
    print(f'{countered_list[-2]} + {countered_list[-1]} = {countered_list[-2] + countered_list[-1]}')
    print('\nEnd\n')


def unpacker_and_counter(my_list):
    counter = 0
    if type(my_list) == float or type(my_list) == int:
        return my_list
    for el in my_list:
        if type(el) == float or type(el) == int:
            counter += el
        elif type(el) == list or type(el) == tuple or type(el) == set or type(el) == frozenset:
            try:
                counter += unpacker_and_counter(el)
            except ValueError:
                return None
            except TypeError:
                return None
...
list_counter([[123, 123, 123], 12, [666, 24, [12, 11, 776]]])
list_counter([1, 2, 3])
list_counter((-127, -361, - 0))
list_counter([1, 2, {0, 1, 4, 81, 64, 9, 16, 49, (5, 4, 3, 2, 1), 25, 36}, [1, 5, 1, 9, [1, 2, 3]]])
