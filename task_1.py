...
def check_1(lst_obj):
    ...
    Сложность: 1 - константа, потсоянная сложность.
    """
...
def check_2(lst_obj):
    ...
    Сложность: 2n+2. линейная
    """
    for j in range(len(lst_obj)):          # n + 1
        if lst_obj[j] in lst_obj[j+1:]:    # 2 тк получаем два объекта
            return False                   # 1 но считается только в случае срабатывания if, а мы следуем худшему варианту и делаем полный перебор
    return True                            # 1
...
def check_3(lst_obj):
    ...
    Сложность: n log n + 2n + 2
    линейно - логарифмическая
    """
    lst_copy = list(lst_obj)                 # 1
    lst_copy.sort()                          # n log n // взял с https://wiki.python.org/moin/TimeComplexity
    for i in range(len(lst_obj) - 1):        # n + 1
        if lst_copy[i] == lst_copy[i+1]:     # 2
            return False                     # 1
    return True                              # 1
